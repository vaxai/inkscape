...
Test executables (`tests`, `unit_tests`) and often `inkscape` are **not** built
by a normal default build (`EXCLUDE_FROM_ALL`). You must build them before
`ctest -L performance`, or use the `performance_tests` target which depends on
those targets.

```bash
# Recommended: build prerequisites and run perf suite
cmake --build build --target tests unit_tests inkscape -j1   # or -j2 on larger VMs
sudo sysctl -w kernel.perf_event_paranoid=-1
ctest --test-dir build -L performance --output-on-failure

# Or one target that depends on tests/unit_tests/inkscape then runs ctest
cmake --build build --target performance_tests -j1

# Single perf test (binary must already exist under build/bin/)
...
If you see `No such file or directory` for `build/bin/test_*` or unit-test
names, the corresponding targets were never compiled — build `tests` /
`unit_tests` / `inkscape` as above.
...
produces a **unique** timestamped SVG (intermediate perf/script/folded/meta files
are discarded after the flame graph is built) so repeated profiling never
