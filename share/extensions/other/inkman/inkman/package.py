...
class PackageItem(object):
    ...
    def is_installable(self, version):
        if self.installed or not self._installer:
            return False
        if not self.verified:
            return False
        if version not in self.targets:
            # wrong Inkscape version
            # can be overridden with environment variable INKMAN_SKIP_VERSION_CHECK=1
            if not os.environ.get("INKMAN_SKIP_VERSION_CHECK"):
                return False
        return True
