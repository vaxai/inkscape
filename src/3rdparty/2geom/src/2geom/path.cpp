...
Coord Path::length(Coord tolerance) const
{
    Coord res = 0;
    if (empty()) { return res;  }
    Coord tol = tolerance / size_default();
    for (auto const &curve : *this) {
        if (!curve.isDegenerate()) {
            res += curve.length(tol);
        }
    }
    return res;
}

PathTime Path::midTime(Coord tolerance) const
{
    PathTime res(0, 0.0);
    if (empty()) { return res; }
    Coord length_remaining = length(tolerance) / 2;
    Coord tol = tolerance / _data->curves.size();   // _data->curves.size() helps getting a better result
    unsigned index = 0;                             // with size_default() the test fails by a tiny margin
    do {
        if (_data->curves[index].isDegenerate()) {
            index++;
            continue;
        }
        Coord cLen = _data->curves[index].length(tol);
        if (length_remaining > cLen) {
            length_remaining -= cLen;
            index++;
        } else {
            double provTime = length_remaining / cLen;
            bool found = false;
            do {
                Curve *tempC = _data->curves[index].portion(0, provTime);
                double portionLen = tempC->length(tol);
                if (are_near(length_remaining, portionLen, tol)) {
                    found = true;
                } else { // at each step ratioDiff will become smaller until we are within the tollerance
                    double ratioDiff = (portionLen / length_remaining) - 1;
                    provTime *= (1 - (ratioDiff / 2)); // half the difference to get progressively closer
                }
                delete tempC;
            } while (found == false);
            res.t = provTime;
            res.curve_index = index;
            return res;
        }
    } while (index < size_default());
    // there was something wrong, it should not get to here
    return res;
}
