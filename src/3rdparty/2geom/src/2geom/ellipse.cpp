...
std::vector<Point> Ellipse::pointAndDerivatives(Coord t, unsigned int n) const
{
    std::vector<Point> result;
    result.resize(n + 1);

    auto const trans = Rotate(t) * Scale(_rays) * Rotate(_angle);

    result[0] = trans.xAxis();

    if (n >= 1) {
        result[1] = trans.yAxis();
    }
