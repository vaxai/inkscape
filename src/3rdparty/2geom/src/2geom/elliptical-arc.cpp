...
EllipticalArc::pointAndDerivatives(Coord t, unsigned int n) const
{
    ...
    auto result = _ellipse.pointAndDerivatives(angleAt(t), n);

    auto const dt = _angles.sweepAngle();
    auto pow = 1.0;
...
#ifdef HAVE_GSL
EllipticalArc EllipticalArc::from_tangents_and_point(Line const &initial_tangent, Point const &point_on_arc,
                                                     Line const &final_tangent)
{
    // The general idea is to find an ellipse through the three given points and with the given tangent lines,
    // and then take a subarc or this ellipse.

    // Create a matrix of linear equations on coefficients A, B, C, D, E, F of the implicit ellipse equation.
    std::unique_ptr<gsl_matrix, decltype(&gsl_matrix_free)> matrix{ gsl_matrix_alloc(5, 6), &gsl_matrix_free };

    // Write in equations coming from the given points on the ellipse:
    unsigned current_row = 0;
    for (auto const [x, y] : { initial_tangent.origin(), point_on_arc, final_tangent.origin() }) {
        gsl_matrix_set(matrix.get(), current_row, 0, x * x);
        gsl_matrix_set(matrix.get(), current_row, 1, x * y);
        gsl_matrix_set(matrix.get(), current_row, 2, y * y);
        gsl_matrix_set(matrix.get(), current_row, 3, x);
        gsl_matrix_set(matrix.get(), current_row, 4, y);
        gsl_matrix_set(matrix.get(), current_row, 5, 1.0);
        ++current_row;
    }

    // Write in equations coming from the tangency conditions
    for (auto const *tangent_line : { &initial_tangent, &final_tangent }) {
        auto const [nx, ny] = rot90(tangent_line->vector());
        auto [x, y] = tangent_line->origin();
        gsl_matrix_set(matrix.get(), current_row, 0, 2.0 * x * ny);
        gsl_matrix_set(matrix.get(), current_row, 1, y * ny - x * nx);
        gsl_matrix_set(matrix.get(), current_row, 2, -2.0 * y * nx);
        gsl_matrix_set(matrix.get(), current_row, 3, ny);
        gsl_matrix_set(matrix.get(), current_row, 4, -nx);
        gsl_matrix_set(matrix.get(), current_row, 5, 0.0);
        ++current_row;
    }

    // Run LU decomposition
    int status = GSL_EFAULT;
    {
        std::unique_ptr<gsl_permutation, decltype(&gsl_permutation_free)> permutation{ gsl_permutation_calloc(5),
                                                                                       &gsl_permutation_free };
        int permutation_sign;
        status = gsl_linalg_LU_decomp(matrix.get(), permutation.get(), &permutation_sign);
    }

    if (status != GSL_SUCCESS) {
        THROW_RANGEERROR("Degenerate elliptical arc fit!");
    }

    // Upper-triangular form matrix accessor helper
    auto const U = [&matrix](int i, int j) -> double { return gsl_matrix_get(matrix.get(), i, j); };

    double coeffs[6] = {std::nan("A"), std::nan("B"), std::nan("C"), std::nan("D"), -U(4, 5), U(4, 4)};

    // Perform back-substitution
    for (int var_idx = 3; var_idx >= 0; --var_idx) {
        double const diagonal_coeff = U(var_idx, var_idx);
        if (diagonal_coeff == 0.0) {
            THROW_RANGEERROR("Degenerate elliptical arc fit!");
        }
        double remaining_terms = 0.0;
        for (int i = var_idx + 1; i < 6; ++i) {
            remaining_terms += coeffs[i] * U(var_idx, i);
        }
        coeffs[var_idx] = -remaining_terms / diagonal_coeff;
    }

    Ellipse resulting_ellipse{ coeffs[0], coeffs[1], coeffs[2], coeffs[3], coeffs[4], coeffs[5] };
    std::unique_ptr<EllipticalArc> const arc{ resulting_ellipse.arc(initial_tangent.origin(), point_on_arc,
                                                                    final_tangent.origin()) };
    return *arc;
}
#endif // HAVE_GSL
