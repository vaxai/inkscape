...
TEST_F(SBasisTest, DISABLED_PathFromSBasis)
{
    auto const B = D2<SBasis>{
        {{{357.753, 421.588}, {582.657, 406.65}, {-1653.21, -1653.21}}},
        {{{536.075, 311.11}, {133.624, -133.048}, {845.823, 845.823}}}
    };

    for (auto const tol : {0.1, 0.01}) {
        auto const path = path_from_sbasis(B, tol);

        // B and path should have small Frechet distance. For simplicity, check a weaker error metric.
        constexpr int N = 20;
        for (int i = 0; i <= N; i++) {
            auto const t = (double)i / N;
            Coord dist;
            path.nearestTime(B.pointAt(t), &dist);
            EXPECT_LE(dist, tol);
        }
    }
}
