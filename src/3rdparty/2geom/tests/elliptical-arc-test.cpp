...
TEST(EllipticalArcTest, PointAndDerivatives)
{
    constexpr int N = 15;
    constexpr double ta = 0.3;
    constexpr double tb = 0.7;
    constexpr double dt = tb - ta;

    for (auto large : {true, false}) {
        for (auto sweep : {true, false}) {
            auto const arc = EllipticalArc{Point{0, 0}, Point{6, 3}, 0.1, large, sweep, Point{10, 0}};

            auto const taylor_series = arc.pointAndDerivatives(ta, N);
            ASSERT_EQ(taylor_series.size(), N + 1);

            Point pt;
            double coeff = 1.0;
            for (int i = 0; i <= N; i++) {
                pt += taylor_series[i] * coeff;
                coeff *= dt / (i + 1);
            }

            EXPECT_near(pt, arc.pointAt(tb), 1e-8);
        }
    }
}
