...
public:
    ...
    /** @brief Calculates the length of the path by adding up the lengths of all curves in the path.
     *         If the path is open, the closing segment is omitted
     * 
     *  @param tolerance Maximum allowed error - smaller values will slow down the calculation
     * 
     *  @return The approximate length of the path within the tolerance */
    Coord length(Coord tolerance = 0.001) const;

    /** @brief Calculates the time of the point in the middle of the path along the length.
     *  The time will correspond to the point that if used to split the path, the resulting 2
     *  paths would have the same length.
     * 
     *  @param tolerance Maximum allowed error - smaller values will slow down the calculation
     * 
     *  @return Returns the PathTime corresponding to the point in the middle of the path */
    PathTime midTime(Coord tolerance = 0.01) const;
