                                  ...
                                  CRAdditionalSel * a_sel, CRXMLNodePtr const a_node)
{
        ...
        while (cur_node) {
                // check if type match (or if type not given)
                // This is wrong but the best we can do without
                // keeping track of types (if sel_name is missing,
                // nth_of_type applies to all types, but separately).
                if (!a_sel->content.pseudo->sel_name ||
                    !strcmp(node_iface->getLocalName(cur_node), a_sel->content.pseudo->sel_name->stryng->str))
                        ++child;
                if (cur_node == a_node) {
                        found = TRUE;
                        break;
                                  ...
                                  CRAdditionalSel * a_sel, CRXMLNodePtr const a_node)
{
        ...
        while (cur_node) {
                // See comment in nth_of_type_pseudo_class_handler().
                if (!a_sel->content.pseudo->sel_name ||
                    !strcmp(node_iface->getLocalName(cur_node), a_sel->content.pseudo->sel_name->stryng->str))
                        ++m;
                if (cur_node == a_node) {
