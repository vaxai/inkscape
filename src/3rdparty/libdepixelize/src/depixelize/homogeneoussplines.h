...
#ifndef LIBDEPIXELIZE_HOMOGENEOUSSPLINES_H
#define LIBDEPIXELIZE_HOMOGENEOUSSPLINES_H
...
public:
    struct Polygon
    {
        ...
        using points_iter = typename Points::iterator;
        using const_points_iter = typename Points::const_iterator;
        using holes_iter = typename std::vector<Points>::iterator;
        using const_holes_iter = typename std::vector<Points>::const_iterator;

        Polygon() {}
        Polygon(const uint8_t (&rgba)[4])
        {
            for ( int i = 0 ; i != 4 ; ++i )
                this->rgba[i] = rgba[i];
        }
    ...
    using size_type = typename std::vector<Polygon>::size_type;
...
private:
    using Points = std::vector<Point<T>>;
    using points_iter = typename Points::iterator;
    using points_citer = typename Points::const_iterator;
    using points_riter = typename Points::reverse_iterator;
    using points_criter = typename Points::const_reverse_iterator;

    using points_range = std::pair<points_iter, points_iter>;
    using points_crange = std::pair<points_citer, points_citer>;
    ...
    _height(voronoi.height())
{
    ...
    using voronoi_citer = SimplifiedVoronoi<T, adjust_splines>::const_iterator;
...
#endif // LIBDEPIXELIZE_HOMOGENEOUSSPLINES_H
